...
class DenseBlock15(nn.Module):
    """DenseNet Block of 15 layers"""
    ...
    def __init__(self, in_channels, k):
        ...
        self.layer13 = Layer(in_channels + k * 12, k)
        self.layer14 = Layer(in_channels + k * 13, k)
        self.layer15 = Layer(in_channels + k * 14, k)
    ...
    def forward(self, x):
        ...
        c12 = torch.cat((c11, x12), dim=1)
        x13 = self.layer13(c12)
        c13 = torch.cat((c12, x13), dim=1)
        x14 = self.layer14(c13)
        c14 = torch.cat((c13, x14), dim=1)
        x15 = self.layer15(c14)
        return torch.cat((x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15), dim=1)
